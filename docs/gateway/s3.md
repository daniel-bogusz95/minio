...
You need valid AWS S3 access key and secret key as a prerequisite to run MinIO S3 gateway by default. Optionally you can also set custom access/secret key, when you have rotating AWS IAM credentials or AWS credentials through environment variables (i.e. AWS_ACCESS_KEY_ID)
...
You need valid access key, secret key, and service endpoint as a prerequisite to run MinIO S3 gateway on an AWS S3 compatible service.
